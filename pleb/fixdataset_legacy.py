...
try:
    ...
    BarycentricTrueEcliptic = None  # type: ignore[assignment]
...
_DEFAULT_JUMPS_PER_SYSTEM = {
    ...
    "GM_GWB_1460_100_b1_pre36.tim": [
        "GM_GWB_1460_100_b1.1372",
        "GM_GWB_1460_100_b1.1397",
        "GM_GWB_1460_100_b1.1422",
        "GM_GWB_1460_100_b1.1447",
    ],
    "GM_GWB_1460_200_b0_post36.tim": [
        "GM_GWB_1460_200_b0.1285",
        "GM_GWB_1460_200_b0.1335",
        "GM_GWB_1460_200_b0.1385",
        "GM_GWB_1460_200_b0.1435",
    ],
    "GM_GWB_500_100_b1_pre36.tim": [
        "GM_GWB_500_100_b1.406",
        "GM_GWB_500_100_b1.418",
        "GM_GWB_500_100_b1.431",
        "GM_GWB_500_100_b1.443",
        "GM_GWB_500_100_b1.456",
        "GM_GWB_500_100_b1.468",
        "GM_GWB_500_100_b1.481",
        "GM_GWB_500_100_b1.493",
    ],
    "GM_GWB_500_200_b1_post36.tim": [
        "GM_GWB_500_200_b1.312",
        "GM_GWB_500_200_b1.337",
        "GM_GWB_500_200_b1.362",
        "GM_GWB_500_200_b1.387",
        "GM_GWB_500_200_b1.412",
        "GM_GWB_500_200_b1.437",
        "GM_GWB_500_200_b1.462",
        "GM_GWB_500_200_b1.487",
    ],
...
_DEFAULT_BACKEND_BW = {
    ...
    "EFF.P200.1380.tim": 400.0,
    "EFF.P217.1380.tim": 400.0,
    "EFF.S110.2487.tim": 400.0,
    "JBO.DFB.1400.tim": 128.0,
    "JBO.DFB.1520.tim": 128.0,
    "JBO.ROACH.1520.tim": 512.0,
    "LEAP.1396.tim": 128.0,
    "NRT.BON.1400.tim": 128.0,
    "NRT.BON.1600.tim": 128.0,
    "NRT.NUPPI.1484.tim": 512.0,
    "NRT.NUPPI.1854.tim": 512.0,
    "SRT.DFB.1400": 128.0,
    "SRT.DFB.330": 64.0,
    "SRT.ROACH.1400": 256.0,
    "SRT.ROACH.330": 128.0,
    "WSRT.P1.1380.2.tim": 64.0,
    "WSRT.P1.1380.C.tim": 64.0,
    "WSRT.P1.1380.tim": 64.0,
    "WSRT.P1.2273.2.tim": 64.0,
    "WSRT.P1.2273.C.tim": 64.0,
    "WSRT.P1.2273.tim": 64.0,
    "WSRT.P1.323.tim": 32.0,
    "WSRT.P1.328.tim": 32.0,
    "WSRT.P1.367.tim": 32.0,
    "WSRT.P1.382.tim": 32.0,
    "WSRT.P2.1380.tim": 128.0,
    "WSRT.P2.2273.tim": 128.0,
    "WSRT.P2.350.tim": 64.0,
    "MK.LBAND.1420.tim": 778.0,
}
...
jumps_per_system["MK.LBAND.1420"] = [
    "MK.LBAND.{:0.0f}".format(meerkat["band_top"] - meerkat["bw"] * (int(x) + 0.5))
    for x in range(meerkat["nchan"])
]  # np.linspace(778,1650,16)]
...
def get_psrs(homepath):
    ...
    """
    return sorted(
        [x.split("/")[-1] for x in glob(os.path.join(homepath, "J?????????"))]
    )
    ...
    for psr in psrs:
        timfiles.append(
            sorted(
                [
                    x.split("/")[-1]
                    for x in glob(os.path.join(homepath, psr, "tims/*.tim"))
                    if "NRT.NUXPI" not in x
                ]
            )
        )
    return timfiles
...
def fix_bad_padd_lines(timfilepath, repo):
    ...
    if repo is not None:
        ...
        repo.index.commit(
            "Fixed bad padd lines in {}".format(timfilepath),
            author=author,
            committer=committer,
        )
    return None
...
def fix_timfiles(homepath, psr, repo):
    ...
    for timfilepath in glob(os.path.join(homepath, psr, "tims", "*.tim")):
        ...
        if len(foundpaddlines) > 0:
            print("{} has bad padd lines. Fixing.".format(timfilepath))
            fix_bad_padd_lines(timfilepath, repo)

        print("mv {} {}".format(timfilepath.replace(".tim", ".tabfixed"), timfilepath))
        os.system(
            "mv {} {}".format(timfilepath.replace(".tim", ".tabfixed"), timfilepath)
        )
        if repo is not None:
            repo.index.add(timfilepath)
            repo.index.commit(
                "Cleaned {}".format(timfilepath), author=author, committer=committer
            )
    return None
...
def skiptimlines(timfilename, my_string="FORMAT", only_once=False, ret_arr=False):
    ...
    """
    lines = np.array(
        findlinesstartingwith(
            searchstring=my_string, filename=timfilename, ret_arr=ret_arr
        )
    )
    ...
    try:
        ...
        if not only_once:
            try:
                if not my_string == "C":
                    skiprows += list(
                        np.arange(foundlines[1] - 1 + offset, rawgencount(filename), 1)
                    )
...
def convert_numeric_cols(dmf, start=1):
    ...
    dmf = dmf[dmf[start + 1] > 10000.0]  # Forced pruning to remove unusual lines
    return dmf
...
def skipper(timfilepath):
    ...
    time_statements_rowidx, time_skip_array = skiptimlines(
        timfilepath, my_string="TIME", ret_arr=True
    )
...
def read_tamfile(
    ...
    **kwargs,
    ...
    time_statements_rowidx, time_skip_array = skiptimlines(
        timfile, my_string="TIME", ret_arr=True
    )
    rowstoskip.extend(time_statements_rowidx)
...
def get_psr_info(homepath, psrs=None, timfiles=None, repo=None):
    ...
    for psr, timfileset in zip(psrs, timfiles):
        ...
        for timfile in timfileset:
            ...
            time_statements_rowidx, time_skip_array = skiptimlines(
                filepath, my_string="TIME", ret_arr=True
            )
            try:
...
def ismatched(freq, sat, keep_freq, keep_mjd, time_window, freq_range):
    ...
    try:
        if (
            isfloat(freq)
            and (freq >= keep_freq - freq_range and freq <= keep_freq + freq_range)
            and isfloat(sat)
            and (sat >= keep_mjd - time_window and sat <= keep_mjd + time_window)
        ):
            return True
        else:
            return False
...
def selectfreqsat(_line, _timfile, freqfrom):
    ...
    if freqfrom == "group":
        ...
        print(
            "Warning: freqfrom can only be group or toa, not {}. Using toa.".format(
                freqfrom
            )
        )
    return _freq, _sat
...
def comment_overlapped_toa(
    homepath, psr, retain_backend, drop_backends, time_window, freqfrom="toa"
):
            ...
            if i not in keeptimskiprows:
                ...
                for timfile in drop_backends:
                    ...
                    if os.path.exists(os.path.join(homepath, psr, "tims/", timfile)):
                        modtimskiprows = skipper(
                            os.path.join(homepath, psr, "tims/", timfile)
                        )
                        mbw = backend_bw.get(timfile)
                        with open(
                            os.path.join(homepath, psr, "tims/", timfile), "r"
                        ) as modtim:
                            with open(
                                os.path.join(
                                    homepath,
                                ...
                                if keep_freq and keep_sat:
                                    for j, modline in enumerate(modtim):
                                        ...
                                        mod_freq, mod_sat = selectfreqsat(
                                            modline, timfile, freqfrom
                                        )
                                        myline = ""

                                        if (
                                            j not in modtimskiprows
                                            and len(modline.split()) > 2
                                            and isfloat(mod_freq)
                                            and ismatched(
                                                mod_freq,
                                                mod_sat,
                                                keep_freq,
                                    ...
                                    os.system(
                                        r"\cp -f {}/{}/tims/{} {}/{}/tims/{}".format(
                                            homepath,
                                            psr,
                                            timfile,
                                            homepath,
                                            psr,
                                            timfile.replace(".tim", ".orig"),
                                        )
                                    )
                                    os.system(
                                        "mv {}/{}/tims/{} {}/{}/tims/{}".format(
...
def remove_nuppi_big(parfile, timfile):
    ...
    with open(parfile, "r") as nuppioldpar, open(timfile, "r") as nuppioldtim:
        with (
            open(parfile.replace(".par", ".new"), "w") as nuppinewpar,
            open(timfile.replace("_all.tim", "_all.new"), "w") as nuppinewtim,
        ):
...
def remove_nuppi_fromParTimfiles(homepath, psr_info, use_newfile=False):
    ...
    for psr in psr_info.keys():
        ...
        try:
            remove_nuppi_big(
                os.path.join(homepath, psr, parfile),
                os.path.join(homepath, psr, timfile),
            )
            if use_newfile:
                os.system(
                    r"\cp -f {}/{}/{} {}/{}/{}".format(
...
def split_on_date(timfilepath, mjd_cut=57500.0, repo=None):
    ...
    with open(timfilepath, "r") as workingfile:
        with (
            open(timfilepath.replace("NUPPI", "NUPPI1"), "w") as nuppiold,
            open(timfilepath.replace("NUPPI", "NUPPI2"), "w") as nuppinew,
        ):
    ...
    if repo is not None:
        repo.index.add(
            [
                timfilepath.replace("NUPPI", "NUPPI1"),
                timfilepath.replace("NUPPI", "NUPPI2"),
            ]
        )
        repo.index.commit(
            "Adding NUPPI split tim files for {}".format(timfilepath.split("/")[-3])
        )
    return None
        ...
        for timfile, flags in timfile.items():
            for flag, value in flags.items():
                if "NUPPI" in value[0] and flag == "-group":
                    ...
                    split_on_date(
                        os.path.join(homepath, psr, "tims", timfile), mjd_cut=57600
                    )


def insert_missing_flags(
    timfilepath,
    flag,
    value,
):
        ...
        with open(timfilepath.replace(".tim", ".new"), "w") as newfile:
            for i, line in enumerate(workingfile):
                ...
                if i in rowstoskip:
                    ...
                    value = "{}.{}".format(
                        timfilepath.split("/")[-1].rstrip(".tim").rsplit(".", 1)[0],
                        _flabel,
                    )
                    line += " {} {}".format(flag, value)
        ...
        for timfile, flags in timfile.items():
            for flag, value in flags.items():
                if value == "missing" and flag == "-sys":
                    print(
                        "WARNING: For PSR {} {} has missing system flags! Check that file is proper.".format(
                            psr, timfile
                        )
                    )
    ...
    with open(alltimfilepath, "r") as workingfile:
        with open(alltimfilepath.replace("_all.tim", "_all.new"), "w") as newfile:
            for line in workingfile:
                ...
                if (
                    line.strip()
                    and ("INCLUDE" == line_contents[0])
                    and (line_contents[1] in addtimfiles)
                ):
                    addtimfiles.remove(line_contents[1])
    with open(alltimfilepath.replace(".tim", ".new"), "a") as newfile:
        for newtims in addtimfiles:
            goodlines = rawgencount(
                os.path.join(alltimfilepath.rpartition("/")[0], "tims", newtims)
            ) - len(
                skipper(
                    os.path.join(alltimfilepath.rpartition("/")[0], "tims", newtims)
                )
            ...
            if goodlines > 9:
                ...
                print(
                    "Dropping {} for {} because it has only {} TOAs".format(
                        newtims, psr, goodlines
                    )
                )
                # newfile.write(line)
...
def update_alltims(homepath, psr_info, use_newfile=False):
    ...
    for psr in psr_info.keys():
        alltim = psr + "_all.tim"
        try:
            insert_missing_timfiles(
                psr,
                alltimfilepath=os.path.join(homepath, psr, alltim),
                psr_info=psr_info,
            )
            if use_newfile:
                os.system(
                    r"\cp -f {}/{}/{} {}/{}/{}".format(
                        ...
                        homepath,
                        psr,
                        alltim.replace("_all.tim", "_all.orig"),
                    )
                )
                os.system(
                    "mv {}/{}/{} {}/{}/{}".format(
                        homepath,
                        psr,
                        alltim.replace("_all.tim", "_all.new"),
                        homepath,
                        psr,
                        alltim,
                    )
    ...
    with open(parfilepath, "r") as workingfile:
        with open(parfilepath.replace(".par", ".new"), "w") as newfile:
            for line in workingfile:
                ...
                line_contents = line.split()
                if (
                    ephem_global is not None
                    and "EPHEM" in line_contents
                    and len(line_contents) > 1
                    and line_contents[1] != ephem_global
                ):
                    line = line.replace(line_contents[1], ephem_global)
                if (
                    bipm_version is not None
                    and line_contents[:1] == ["CLK"]
                    and len(line_contents) > 1
                    and line_contents[1] != bipm_version
                ):
                    line = line.replace(line_contents[1], bipm_version)
                if (
                    nesw_value is not None
                    and "NE_SW" in line_contents
                    and len(line_contents) > 1
                    and line_contents[1] != nesw_value
                ):
    ...
    for psr in psr_info.keys():
        ...
        try:
            insert_missing_jumps(
                psr, parfilepath=os.path.join(homepath, psr, parfile), psr_info=psr_info
            )
            if use_newfile:
                os.system(
                    r"\cp -f {}/{}/{} {}/{}/{}".format(
...
def equatorial_to_ecliptic_par(parfile):
    ...
    try:
        ...
        radec = SkyCoord(
            ...
            pm_ra_cosdec=float(pmra[0][1]) * units.mas / units.yr,
            pm_dec=float(pmdec[0][1]) * units.mas / units.yr,
            equinox="J2000",
            obstime=Time(float(posepoch[0][1]), format="mjd"),
        )
        ...
        drop_lines = [
            rem
            for rem, line in enumerate(lines)
            for eqterm in eqterms
            if eqterm in line
        ]
    ...
    try:
        ...
        with open(parfile.replace(".par", "_eq.par"), "w") as eqpar:
            for line in lines:
                ...
                if "ELONG" in line and "PM" not in line:
                    line = "{:<15}{:<15} 1 0.00000".format("RAJ", raj)
                if "ELAT" in line and "PM" not in line:
        ...
        if use_newfile:
            os.system(
                r"\cp -f {} {}".format(
                    parfile, parfile.replace(".par", "_dr1eqecl.par")
                )
            )
...
def check_all_binary(parfile_lines):
    ...
    if (
        "PB" in parfile_lines
        and "A1" in parfile_lines
        and "PBDOT" in parfile_lines
        and ("XDOT" in parfile_lines or "A1DOT" in parfile_lines)
    ):
        return True
    else:
        return False
...
def add_params(psr, parfilepath, psr_info, params, start_line):
    ...
    with open(parfilepath, "r") as workingfile:
        ...
        if (
            "{}M2".format(os.linesep) in parfile_lines or "SINI" in parfile_lines
        ) and "H3" in params:
            params.remove("H3")
            params.remove("STIG")
            pkpars = ["M2", "SINI"]
            ...
            for line in workingfile:
                if line_no == start_line:
                    for newparam in params:
                        newfile.write(
                            "{:<15}0    1    0{}".format(newparam, os.linesep)
                        )
                newfile.write(line)
                line_no += 1
...
def add_params_to_parfile(
    homepath,
    psr_info,
    newpars,
    use_newfile=False,
    branch_message="Auto commit",
    repo=None,
):
    ...
    for psr in psr_info.keys():
        parfile = psr + ".par"
        start_line = findlinesstartingwith(
            searchstring="TZRFRQ", filename=os.path.join(homepath, psr, parfile)
        )[0]
        added_params = add_params(
            psr,
            parfilepath=os.path.join(homepath, psr, parfile),
            psr_info=psr_info,
            params=list(newpars),
        ...
        if use_newfile:
            ...
            os.system(
                "mv {}/{}/{} {}/{}/{}".format(
                    homepath,
                    psr,
                    parfile.replace(".par", "_addpar.par"),
                    homepath,
                    psr,
                    parfile,
                )
            ...
            if repo is not None:
                ...
                repo.index.commit(
                    message="{} Added {} for {}".format(
                        branch_message,
                        " and ".join(", ".join(added_params).rsplit(", ", 1)),
                        psr,
                    )
                )
            print(
