...
def _choose(cli_value, config_value, default=None):
    return (
        ...
        else (config_value if config_value is not None else default)
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional TOML/JSON release-quality-report config.",
    )
    p.add_argument(
        "--report-dir",
        type=Path,
        default=None,
        help="Output directory; default is <run-dir>/release_quality_report.",
    )
    p.add_argument(
        "--output-name",
        default=None,
        help="PDF filename; default is release_quality_report.pdf.",
    )
    p.add_argument("--title", default=None, help="Report title override.")
    p.add_argument(
        "--backend-col", default=None, help="Preferred backend column for risk ranking."
    )
    p.add_argument(
        "--outlier-cols",
        default=None,
        help="Comma-separated compact-decision outlier columns.",
    )
    p.add_argument(
        "--no-per-pulsar-pages", action="store_true", help="Skip residual-vs-MJD pages."
    )
    p.add_argument(
        "--per-pulsar-page-limit",
        type=int,
        default=None,
        help="Maximum number of per-pulsar pages.",
    )
    p.add_argument(
        "--top-n",
        type=int,
        default=None,
        help="Number of flagged TOA rows in the compact table.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    run_dir = (
        args.run_dir if args.run_dir is not None else (cfg.run_dir if cfg else None)
    )
    if run_dir is None:
    ...
    thresholds = ReleaseQualityThresholds(
        yellow_bad_fraction=float(
            _choose(
                args.yellow_bad_fraction, cfg.yellow_bad_fraction if cfg else None, 0.01
            )
        ),
        red_bad_fraction=float(
            _choose(args.red_bad_fraction, cfg.red_bad_fraction if cfg else None, 0.05)
        ),
        yellow_review_fraction=float(
            _choose(
                args.yellow_review_fraction,
                cfg.yellow_review_fraction if cfg else None,
                0.005,
            )
        ),
        red_review_fraction=float(
            _choose(
                args.red_review_fraction, cfg.red_review_fraction if cfg else None, 0.02
            )
        ),
        yellow_event_fraction=float(
            _choose(
                args.yellow_event_fraction,
                cfg.yellow_event_fraction if cfg else None,
                0.10,
            )
        ),
    )
    include_per_pulsar_pages = (
        False
        if args.no_per_pulsar_pages
        else (cfg.include_per_pulsar_pages if cfg else True)
    )
    ...
    result = generate_release_quality_report(
        ...
        report_dir=(
            ...
            else (cfg.report_dir if cfg else None)
        ),
        output_name=str(
            _choose(
                args.output_name,
                cfg.output_name if cfg else None,
        ...
        outlier_cols=(
            ...
            else (cfg.outlier_cols if cfg else None)
        ),
        ...
        per_pulsar_page_limit=int(
            _choose(
                ...
                cfg.per_pulsar_page_limit if cfg else None,
                30,
            )
        ),
