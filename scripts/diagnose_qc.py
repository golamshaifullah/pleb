...
import argparse
import pandas as pd
from pqc.utils.diagnostics import (
    summarize_dataset,
    summarize_results,
    export_structure_table,
)
...
def main() -> None:
    ...
    for cols in groupings:
        ...
        if struct.empty:
            ...
            continue
        present = struct[struct["present"].fillna(False)]
        if not present.empty:
            info(
                f"Structure diagnostics for grouping [{label}] (present=True, first 30):"
            )
            info(present.head(30).to_string(index=False))
        else:
            info(
                f"Structure diagnostics for grouping [{label}] (no present=True; showing first 30):"
            )
